...
async def cmd_start(message: Message):
    name = message.from_user.first_name or "Администратор"
    await message.answer(
        f"⭐ <b>StarvellBot</b>\n\n"
        f"Привет, <b>{name}</b>! Добро пожаловать в панель управления.\n\n"
        f"⚙️ <b>Управление</b>\n"
        f"  /services — службы\n"
        f"  /status — статус служб\n\n"
        f"🛠 <b>Конфигурация</b>\n"
        f"  /config — настройки\n\n"
        f"📊 <b>Аналитика</b>\n"
        f"  /stats — продажи и сессии\n"
        f"  /list — неподтв. заказы >12ч\n\n"
        f"  /help — справка",
        parse_mode="HTML"
    )
...
async def cmd_help(message: Message):
    await message.answer(
        f"📖 <b>Справка</b>\n\n"
        f"⚙️ <b>Управление службами</b>\n"
        f"  /services — запуск / стоп / рестарт\n"
        f"  /status — uptime и ошибки\n\n"
        f"🛠 <b>Конфигурация</b>\n"
        f"  /config — редактор настроек\n"
        f"  Навигация: раздел → поле → значение\n"
        f"  Изменения сохраняются автоматически\n\n"
        f"📊 <b>Аналитика</b>\n"
        f"  /stats — выданные ⭐, балансы сессий, флудвейты\n"
        f"  /list — заказы без подтверждения старше 12ч",
        parse_mode="HTML"
    )
